...
#   CGO_ENABLED=0 go build -o mcp-timescale-linux-amd64 .
FROM gcr.io/distroless/static-debian12:nonroot
ARG TARGETOS
ARG TARGETARCH
COPY mcp-timescale-${TARGETOS}-${TARGETARCH} /usr/local/bin/mcp-timescale
USER nonroot:nonroot
EXPOSE 8080 9091
ENTRYPOINT ["/usr/local/bin/mcp-timescale"]
CMD ["serve"]
