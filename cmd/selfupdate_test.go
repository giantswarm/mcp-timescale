...
func TestSelfUpdateCmdRefusesDevelopmentVersions(t *testing.T) {
	for _, version := range []string{"dev", ""} {
		t.Run("version="+version, func(t *testing.T) {
			...
			err := newSelfUpdateCmd().Execute()
			if err == nil || !strings.Contains(err.Error(), "cannot self-update a development version") {
				t.Fatalf("want the development-version refusal, got %v", err)
			}
		})
	}
	// Actual updates need network access and real releases, which is not
	// unit-test territory; the wiring below uses a fake release source.
}
...
func TestSelfUpdateCmdProperties(t *testing.T) {
	...
	if cmd.Short != "Update mcp-timescale to the latest version" {
		t.Errorf("Short = %q", cmd.Short)
	}
	for _, want := range []string{"mcp-timescale", "GitHub", "Sigstore bundle"} {
		if !strings.Contains(cmd.Long, want) {
			t.Errorf("the help should mention %q (releases are verified), got:\n%s", want, cmd.Long)
		}
	}
	if githubRepoSlug != "giantswarm/mcp-timescale" {
		t.Errorf("githubRepoSlug = %q", githubRepoSlug)
	}
}
...
// mcp-timescale wires it in so that an unsigned or unverifiable release never
// reaches the disk.
...
func (r fakeRelease) GetURL() string {
	return "https://github.com/giantswarm/mcp-timescale/releases/tag/" + r.tag
}
...
func binaryAsset() string {
	name := "mcp-timescale-" + runtime.GOOS + "-" + runtime.GOARCH
...
func selfUpdateFixture(t *testing.T, src *fakeSource) (string, []byte) {
	t.Helper()
	installed := []byte("the mcp-timescale that is installed right now")
	exe := filepath.Join(t.TempDir(), "mcp-timescale")
...
func TestRunSelfUpdateRefusesAReleaseWithoutASignatureBundle(t *testing.T) {
	src := &fakeSource{
		...
		assets:  map[int64][]byte{1: []byte("a newer mcp-timescale, unsigned")},
	}
...
func TestRunSelfUpdateInstallsAVerifiedReleaseInPlace(t *testing.T) {
	...
	src := &fakeSource{
		...
		assets: map[int64][]byte{1: []byte("a newer mcp-timescale"), 2: []byte("its bundle")},
	}
	...
	got, err := os.ReadFile(exe) //nolint:gosec // the test's own temp file
	if err != nil || string(got) != "a newer mcp-timescale" {
