...
Every [release](https://github.com/giantswarm/mcp-timescale/releases) carries
one binary per platform (`mcp-timescale-<os>-<arch>`: linux, darwin and
windows on amd64 and arm64) next to its cosign Sigstore bundle
(`<asset>.bundle`), the multi-arch image
`gsoci.azurecr.io/giantswarm/mcp-timescale:<version>` and the chart in the
Giant Swarm catalog.

```bash
# a release binary (verify by hand with cosign v3: verify-blob --bundle <asset>.bundle)
curl -fsSLo mcp-timescale https://github.com/giantswarm/mcp-timescale/releases/latest/download/mcp-timescale-linux-amd64
chmod +x mcp-timescale

# or from source
go install github.com/giantswarm/mcp-timescale@latest
```

An installed binary keeps itself current:

```bash
mcp-timescale self-update
```

`self-update` downloads the latest release's binary together with its bundle
and installs it only after the bundle verifies for a CircleCI build of
`giantswarm/mcp-timescale` against the Sigstore public-good trust root. A
release without a bundle, or a download that does not match its signature, is
refused and the installed binary stays as it is.

## CLI

```text
mcp-timescale               # same as `mcp-timescale serve`
mcp-timescale serve         # --transport stdio|sse|streamable-http, --mcp-addr, --metrics-addr, --databases-file, --debug
mcp-timescale version       # version, commit and build time
mcp-timescale self-update   # verified update to the latest release
```

`version` and `--version` report the release version; a plain `go build`
reports Go's VCS build info (`+dirty` with untracked files), the generated
...
| Path | Purpose |
| --- | --- |
| `cmd/` | cobra entry — `serve.go` wires config, pools, tools and transports; `version`, `self-update` |
...
| `helm/mcp-timescale/` | Helm chart; `tests/` are helm-unittest suites, `ci/ci-values.yaml` renders every feature |
| `scripts/integration-test.sh` | Docker-backed integration run (`make test-integration`) |

`make build` (stamps the version), `make test`, `make lint` and `make fmt` come
from the generated Makefile. `make test-integration` runs the Docker-backed
integration suite, `make helm-lint` / `make helm-template` render the chart
with `helm/mcp-timescale/ci/ci-values.yaml`, `make helm-test` runs the chart
unit tests and `make govulncheck` scans the module graph.
`pre-commit run --all-files` is what CI runs: gofmt, goimports, golangci-lint
(gosec, goconst), `go mod tidy`, the values schema and helm-docs.
`.cursor/rules/` carries the architecture invariants and this workflow for
agents working in the repo.

## Releasing

Releases are cut automatically from Conventional Commit PR titles (`feat:`,
`fix:`, `feat!:`; `chore:` and `ci:` bump the patch too, `docs:` releases
nothing): the Auto Release workflow tags the merge commit, and the CircleCI
tag pipeline attaches the six platform binaries with their cosign bundles to
the GitHub Release, pushes the multi-arch image to gsoci and the chart to the
Giant Swarm catalog. The CI itself is generated by devctl from this repo's
entry in giantswarm/github (`gen.flavours: generic, app, cli`) and kept in
sync by the align-files workflow — change it there, not in the generated
files.
